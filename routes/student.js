...
router.get('/notifications', studentOnly, async (req, res) => {
  try {
    const userId = req.session.user_id;
    const schoolCode = req.session.school_code || 'GLOBAL';
    let notifications = [];
    try {
      notifications = (await pool.query(
        "SELECT * FROM notifications WHERE (user_id = $1 OR school_code = $2 OR school_code = 'GLOBAL') ORDER BY created_at DESC LIMIT 40",
        [userId, schoolCode]
      )).rows || [];
    } catch (e) {
      try {
        notifications = (await pool.query(
          "SELECT * FROM announcements WHERE school_code = $1 OR school_code = 'GLOBAL' ORDER BY created_at DESC LIMIT 20",
          [schoolCode]
        )).rows || [];
      } catch (e2) { notifications = []; }
    }
  ...
  try {
    ...
    try {
      events = (await pool.query(
        "SELECT * FROM library_events WHERE (school_code = $1 OR school_code = 'GLOBAL') ORDER BY event_date ASC LIMIT 30",
        [schoolCode]
      )).rows || [];
    } catch (e) {
      try {
        events = (await pool.query(
          'SELECT * FROM events WHERE school_code = $1 ORDER BY event_date ASC LIMIT 30',
          [schoolCode]
        )).rows || [];
      } catch (e2) { events = []; }
    }
    // Also add due-date events from transactions
    let dueDates = [];
    try {
      dueDates = (await pool.query(
        "SELECT b.title, t.due_date as event_date, 'due' as category, ('Due: ' || b.title) as description FROM transactions t JOIN books b ON b.id = t.book_id WHERE t.user_id = $1 AND t.return_date IS NULL AND t.due_date IS NOT NULL ORDER BY t.due_date ASC",
        [userId]
      )).rows || [];
    } catch (e) { dueDates = []; }
    events = [...events, ...dueDates];
...
router.post('/requests', studentOnly, async (req, res) => {
  const { title, author, request_type, details } = req.body;
  if (!title) {
    if (req.flash) req.flash('error', 'Book title is required.');
  ...
  try {
    await pool.query(
      ...
      [req.session.user_id, title, author || '', request_type || 'book', details || '', req.session.school_code || 'GLOBAL', new Date().toISOString()]
    );
    if (req.flash) req.flash('success', 'Book request submitted!');
    res.redirect('/student/requests');
  } catch (err) {
    console.error('Request create error:', err);
    if (req.flash) req.flash('error', 'Failed to submit request.');
    res.redirect('/student/requests');
  }
});
  ...
  try {
    ...
    try {
      assignments = (await pool.query(
        "SELECT * FROM assignments WHERE (school_code = $1 OR school_code = 'GLOBAL') ORDER BY due_date ASC",
        [schoolCode]
      )).rows || [];
    } catch (e) { assignments = []; }
    try {
